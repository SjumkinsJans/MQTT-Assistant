...
void turn_on(struct mosquitto* mosq,char * message,char * command,struct device_name_pair **device_name,int *max_pair_count,struct Device **devices,int max_device_count) {
    printf("executing : turn on\n");
    ...
    switch(type) {
        ...
        case ZIGBEE:
            ...
            sprintf(payload,"{\"Device\":\"%s\",\"Send\":{\"Power\":1}}",shortaddr);
...
// "cmnd/ZBridge_38B2D9/ZbSend" -m '{"Device":"0x712B","Send":{"Power":0}}'